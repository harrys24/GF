            ...
            $.post('/finance/findByNie', {txt:txt}, function(res){
                ...
                for(i in recu){
                    ...
                    if(res.type == "devmaster"){
                        tr += '<tr class="'+cls+'"><td>'+recu[i].idR+'</td><td>'+recu[i].NIE+'</td><td>'+recu[i].nom+'</td><td>'+recu[i].prenom+'</td><td>'+(recu[i].nom_niv+""+recu[i].nom_gp)+'</td><td class="text-center">'+($.date(new Date(recu[i].date_p)))+'</td><td>'+format(parseFloat(recu[i].montant).toFixed(2))+' Ar</td><td>'+format(parseFloat(recu[i].reste).toFixed(2))+' Ar</td><td>'+recu[i].mode+'</td><td>'+recu[i].designation+'</td><td class="d-flex justify-content-around flex-lg-row flex-column"><button class="btn btn-outline-warning btn-warning btn-light" id="modif" data-idr='+recu[i].idR+'><i class="bi bi-pen"></i></button><button class="btn btn-outline-danger btn-light" id="delete" data-idr="'+recu[i].idR+'"><i class="bi bi-trash"></i></button><a href="./print/'+recu[i].idR+'" target="_blank" class="btn btn-outline-admin btn-light mt-lg-0 mt-1"><i class="bi bi-printer"></i></a></td></tr>';
                    }else{
                        if(recu[i].annul_recu == 1){
                            tr += '<tr class="'+cls+'"><td>'+recu[i].idR+'</td><td>'+recu[i].NIE+'</td><td>'+recu[i].nom+'</td><td>'+recu[i].prenom+'</td><td>'+(recu[i].nom_niv+""+recu[i].nom_gp)+'</td><td class="text-center">'+($.date(new Date(recu[i].date_p)))+'</td><td>'+format(parseFloat(recu[i].montant).toFixed(2))+' Ar</td><td>'+format(parseFloat(recu[i].reste).toFixed(2))+' Ar</td><td>'+recu[i].mode+'</td><td>'+recu[i].designation+'</td><td class="d-flex justify-content-around flex-lg-row flex-column"></td></tr>';
                        }else{
